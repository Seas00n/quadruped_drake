...
class MPTCController(BasicController):
    ...
    def ControlLaw(self, context, q, v):
        ...
        pdd_feet_nom = np.array(
            [trunk_data["pdd_lf"], trunk_data["pdd_rf"], trunk_data["pdd_lh"], trunk_data["pdd_rh"]])
        ...
        W = np.diag(
            np.hstack([w_body * np.ones(6), w_foot * np.ones(nf)]))  # Note: premultiplying by Lambda^{-1} ensures
